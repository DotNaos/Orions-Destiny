...
public class EditorCamera extends Component {
    ...
    public void updateEditor(float dt) {
        ...
        if (MouseListener.getScrollY() != 0.0f) {
            float addValue = (float) Math.pow(Math.abs(MouseListener.getScrollY() * scrollSensitivity), 1 / levelEditorViewport.getZoom());
        ...
        if (reset) {
            ...
            levelEditorViewport.setZoom(this.levelEditorViewport.getZoom() + ((1.0f - levelEditorViewport.getZoom()) * lerpTime));
            this.lerpTime += 0.1f * dt;
            if (Math.abs(levelEditorViewport.position.x) <= 5.0f && Math.abs(levelEditorViewport.position.y) <= 5.0f) {
                this.lerpTime = 0.0f;
