...
public class EditorCamera extends Component
{
    ...
    private final int BUTTON = GLFW_MOUSE_BUTTON_MIDDLE;
    ...
    public void update(float dt)
    {
        ...
        if (reset)
        {
            ...
            if (Math.abs(levelEditorCamera.position.x) <= 1.0f && Math.abs(levelEditorCamera.position.y) <= 1.0f && Math.abs(levelEditorCamera.getZoom() - 1.0f) <= 0.01f)
            {
                this.lerpTime = 0.0f;
